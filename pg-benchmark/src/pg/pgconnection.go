...
func NewPgConnection(pgConfig *conf.PgServerConfig) (*PgConnection, error) {
	pgConInfo := fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=disable", pgConfig.Host, pgConfig.Port, pgConfig.User, pgConfig.PassWord, pgConfig.DbName)
