...
inline int64_t vector_size(vector* v)
{
    return v->cursize;
}

static void quick_sort(vector* v, void* item[], int64_t left, int64_t right)
{
    if (left < right && v->ccb != NULL && v->cursize > 0) {
        ...
        item[i] = tmp;
        quick_sort(v, v->item, left, i - 1);
        quick_sort(v, v->item, i + 1, right);
    }
}

inline void vector_sort(vector* v)
{
    quick_sort(v, v->item, 0, v->cursize - 1);
}

static inline int vector_mutex_lock(vector* v)
{
    return pthread_mutex_lock(&v->mutex);
}

static inline int vector_mutex_unlock(vector* v)
{
...
static void vector_expand(vector* v)
{
    ...
    v->maxsize = size;
    v->oversize = (int64_t)(v->maxsize * VECTOR_EXPAND_P);
}

vector* vector_create(int64_t maxsize, free_callback fcb, cmp_callback ccb)
{
    ...
    assert(v->item != NULL);
    v->oversize = (int64_t)(maxsize * VECTOR_EXPAND_P);
...
int main(int argc, char* args[])
{
    ...
    vector* v = vector_create(tmp, NULL, &cmp);
    ...
    //vector_sort (v);
    for (int64_t i = 0; i < tmp; i++) {
        uint32_t* data = (uint32_t*)vector_get(v, i);
